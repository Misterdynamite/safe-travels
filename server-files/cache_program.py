...
# API key (can be set via env var AT_API_KEY to avoid committing secrets)
API_KEY = os.environ.get("AT_API_KEY", "25c926c6234a49c98d52d90a8bd7ac7e")
GTFS_BASE_URL = os.environ.get("GTFS_BASE_URL", "https://api.at.govt.nz/gtfs/v3")
GTFS_STATIC_URL = os.environ.get("GTFS_STATIC_URL", "https://gtfs.at.govt.nz/gtfs.zip")

# PostgreSQL connection details (override with env vars in production)
DB_CONFIG = {
...
# Working directory for GTFS files (platform independent default)
GTFS_WORK_DIR = os.environ.get("GTFS_WORK_DIR", os.path.join(tempfile.gettempdir(), "gtfs_data"))
...
def log(message, level="info"):
    """Simple logging wrapper (keeps signature similar to original)."""
    if level == "error":
